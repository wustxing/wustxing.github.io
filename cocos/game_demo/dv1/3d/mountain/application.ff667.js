    ...
    execute: function () {
      _export("Application", Application = /*#__PURE__*/function () {
        function Application() {
          ...
          this.settingsPath = 'src/settings.84f30.json'; // settings.json 文件路径，通常由编辑器构建时传入，你也可以指定自己的路径

          this.showFPS = true; // 是否打开 profiler, 通常由编辑器构建时传入，你也可以指定你需要的值
        }
        ...
        _createClass(Application, [{
          ...
          key: "onPostInitBase",
          value: function onPostInitBase() {//-基础模块初始化(logging, sys, settings)
            // cc.settings.overrideSettings('assets', 'server', '');
            // 实现一些自定义的逻辑
          }
        }, {
          key: "onPostSystemInit",
          value: function onPostSystemInit() {//-子系统模块初始化(animation, physics, tween, ui, middleware 等)
            // 实现一些自定义的逻辑
          }
        }, {
          key: "start",
